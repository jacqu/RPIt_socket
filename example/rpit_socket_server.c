...
int sock_sensorfd; // socket sensor file descriptor
...
#define RPIT_SOCKET_PORT           "31418" // Port of the server
...
void *rpit_socket_server_update( void *ptr ) {
     ...
     int numbytes;
     struct sockaddr_storage mes_addr;
     char buf[MAXBUFLEN];
     socklen_t mes_addr_len;
     /* char s[INET6_ADDRSTRLEN]; */

     mes_addr_len = sizeof mes_addr;
     double x, y, z;
     ...
     while( 1 ) {
	  ...
	  if ((numbytes = recvfrom(sock_sensorfd, buf, MAXBUFLEN-1 , 0,
				   (struct sockaddr *)&mes_addr, &mes_addr_len)) == -1) {
	       flockfile( stderr );
	       fprintf( stderr, "failed to receive sensor messages\n");
	       funlockfile( stderr );
	  }
	  buf[numbytes] = '\0';

	  /* flockfile( stderr ); */
	  /* fprintf( stderr, "Sensor package received: %s", buf); */
	  /* funlockfile( stderr ); */


	  if( parse_total_station_msg( buf, &x, &y, &z) ) {
	       flockfile( stderr );
	       fprintf( stderr, "failed to parse sensor message\n");
	       funlockfile( stderr );
	  }
...
int main( void ) {
     ...
     socket_configuration( &sock_sensorfd );
